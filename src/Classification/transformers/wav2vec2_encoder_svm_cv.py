...
RESULTS_DIR = "experiments/exp_03_wav2vec2_encoder_svm"
RESULTS_PATH = os.path.join(RESULTS_DIR, "results.txt")

EMBEDDINGS_OUT = "data/processed/embeddings/wav2vec2_base_embeddings.npz"
MODEL_OUT = "models/wav2vec2_encoder_svm/wav2vec2_encoder_svm.joblib"
...
def load_audio(audio_path: str, sr: int = 16000) -> np.ndarray:
    ...
    wav2vec2 expects 16 kHz input audio. We use librosa here to keep
    the preprocessing consistent with the other experiments.
    """
...
def extract_wav2vec2_embedding(audio, processor, model, device) -> np.ndarray:
    """
    Extract one fixed-size embedding from wav2vec2.
    ...
    The final result is one embedding vector per audio file.
    """
...
def build_embeddings():
    """
    Read labels_clean.csv and extract wav2vec2 embeddings for all available audios.
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Extracting wav2vec2 embeddings"):
...
def load_or_build_embeddings():
    """
    Load existing wav2vec2 embeddings if available, otherwise compute them.

    Important:
    If labels_clean.csv changes, it is safer to delete the old .npz file
    before running this script. This script prints the number of loaded
    samples to make old caches easy to detect.
    """

    os.makedirs(os.path.dirname(EMBEDDINGS_OUT), exist_ok=True)

    if os.path.exists(EMBEDDINGS_OUT):
        print("Loading existing embeddings:", EMBEDDINGS_OUT)
    ...
    print("No saved embeddings found. Extracting embeddings...")
...
def filter_classes_with_enough_samples(X, y, filenames, min_samples=2):
    ...
    With n_splits=2, each evaluated class must have at least 2 samples.

    The full labels_clean.csv remains unchanged. Filtering only affects
...
def run_cross_validation(X, y):
    ...
    wav2vec2 embeddings and select the best one using Macro F1.
    """
...
def save_results(
    ...
    with open(RESULTS_PATH, "w", encoding="utf-8") as f:
        f.write("Experiment: wav2vec2 Encoder + SVM\n")
        f.write("==================================\n\n")

        f.write("Description:\n")
        f.write(
            "This experiment uses wav2vec2 as a frozen feature extractor. "
        ...
        f.write(f"- wav2vec2 model: {WAV2VEC2_MODEL_NAME}\n")
...
def main():
    ...
    print("Loading or extracting wav2vec2 embeddings...")
    ...
    joblib.dump({
        ...
        "feature_type": "wav2vec2_encoder_mean_pooling",
