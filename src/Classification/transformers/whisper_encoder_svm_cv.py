...
RESULTS_DIR = "experiments/exp_02_whisper_encoder_svm"
RESULTS_PATH = os.path.join(RESULTS_DIR, "results.txt")

EMBEDDINGS_OUT = "data/processed/embeddings/whisper_tiny_embeddings.npz"
MODEL_OUT = "models/whisper_encoder_svm/whisper_encoder_svm.joblib"
...
def load_audio(audio_path: str, sr: int = 16000) -> np.ndarray:
    ...
    Whisper expects 16 kHz audio. We use librosa here to keep the
    preprocessing consistent across the experiments.
    """
...
def extract_whisper_embedding(audio, processor, model, device) -> np.ndarray:
    ...
    The final output is one vector per audio file.
    """
...
def build_embeddings():
    """
    Read labels_clean.csv and extract Whisper embeddings for all available audios.
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Extracting Whisper embeddings"):
...
def load_or_build_embeddings():
    """
    Load existing Whisper embeddings if available, otherwise compute them.

    Important:
    If labels_clean.csv changes, it is safer to delete the old .npz file
    before running this script. This script also prints the number of loaded
    samples so the user can quickly detect old cached embeddings.
    """
...
def run_cross_validation(X, y):
    ...
    We keep this experiment readable:
    - StandardScaler
    - SVM classifier
    - several kernels / C values
...
def save_results(
    ...
    with open(RESULTS_PATH, "w", encoding="utf-8") as f:
        f.write("Experiment: Whisper Encoder + SVM\n")
        f.write("=================================\n\n")

        f.write("Description:\n")
        f.write(
            "This experiment uses the Whisper encoder as a frozen feature extractor. "
            "For each audio file, the encoder hidden states are mean-pooled over time "
...
def main():
    ...
    print("Loading or extracting Whisper embeddings...")
    ...
    joblib.dump({
        ...
        "feature_type": "whisper_encoder_mean_pooling",
